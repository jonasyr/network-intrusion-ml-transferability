...
def save_checkpoint(
    pipeline: Pipeline, checkpoint_path: Path, progress_info: Dict
) -> None:
...
def create_balanced_batch(
    chunk: pd.DataFrame, batch_size: int = 20000
    ...
    balanced_batch = pd.concat([benign_sample, malicious_sample]).sample(
...
def compute_adaptive_class_weights(y: np.ndarray) -> Dict[int, float]:
    ...
    weight_dict = {
        int(cls): float(weight) for cls, weight in zip(unique_classes, class_weights)
    }
...
def train_on_full_cic_dataset() -> Pipeline:
    ...
    for file_idx, file_path in enumerate(sorted(csv_files)):
        try:
                    ...
                    if processed_chunk is not None and len(processed_chunk) > 0:
                        ...
                        if len(np.unique(y_chunk)) >= 2:
                            if not is_fitted:
                                ...
                                print(
                                    f"    Initial fit on chunk {chunk_num} ({len(X_chunk):,} samples)"
                                )
                                print(
                                    f"    Class distribution: {benign_count} benign, {malicious_count} malicious"
                                )
                                ...
                                test_f1 = f1_score(
                                    y_chunk, y_pred_test, zero_division=0
                                )
                                print(
                                    f"    Initial training: Acc={test_acc:.3f}, F1={test_f1:.3f}"
                                )
                                ...
                                cumulative_total = (
                                    total_benign_seen + total_malicious_seen
                                )
                                if cumulative_total > 0:
                                    benign_ratio = total_benign_seen / cumulative_total
                                    malicious_ratio = (
                                        total_malicious_seen / cumulative_total
                                    )
                                    ...
                                    sample_weights = np.where(
                                        y_chunk == 0,
                                        1.0
                                        / (benign_ratio + 1e-8),  # Weight for benign
                                        1.0 / (malicious_ratio + 1e-8),
                                    )  # Weight for malicious
                                ...
                                if chunk_num % 5 == 0:  # Print less frequently
                                    print(
                                        f"    Partial fit on chunk {chunk_num} ({len(X_chunk):,} samples)"
                                    )
                                    print(
                                        f"    Class distribution: {benign_count} benign, {malicious_count} malicious"
                                    )
                                    print(
                                        f"    Sample weight range: [{np.min(sample_weights):.3f}, {np.max(sample_weights):.3f}]"
                                    )
                                ...
                                pipeline.named_steps["clf"].partial_fit(
                                    X_transformed, y_chunk, sample_weight=sample_weights
                                )
                                ...
                                if chunk_num % 20 == 0:
                                    ...
                                    val_f1 = f1_score(
                                        y_chunk, y_pred_val, zero_division=0
                                    )
                                    ...
                                    print(
                                        f"    Validation (chunk {chunk_num}): Acc={val_acc:.3f}, F1={val_f1:.3f}"
                                    )
                                    print(
                                        f"    Predictions: {pred_benign} benign, {pred_malicious} malicious"
                                    )
                            ...
                            if chunk_num % 10 == 0:
                                print(
                                    f"    Skipping chunk {chunk_num} (only {benign_count} benign, {malicious_count} malicious)"
                                )
                    ...
                    if processed_df is not None and len(processed_df) > 0:
                        ...
                        for i in range(0, len(processed_df), BATCH_SIZE):
                            ...
                            if len(np.unique(y_batch)) >= 2:
                                if not is_fitted:
                                    # Compute adaptive class weights for initial fit
                                    class_weights = compute_adaptive_class_weights(
                                        y_batch
                                    )
                                    ...
                                    print(
                                        f"    Initial fit on batch {i//BATCH_SIZE + 1} ({len(X_batch):,} samples)"
                                    )
                                    print(
                                        f"    Class distribution: {benign_count} benign, {malicious_count} malicious"
                                    )
                                    ...
                                    test_f1 = f1_score(
                                        y_batch, y_pred_test, zero_division=0
                                    )
                                    print(
                                        f"    Initial training: Acc={test_acc:.3f}, F1={test_f1:.3f}"
                                    )
                                    ...
                                    if cumulative_total > 0:
                                        benign_ratio = (
                                            total_benign_seen / cumulative_total
                                        )
                                        malicious_ratio = (
                                            total_malicious_seen / cumulative_total
                                        )
                                ...
                                print(
                                    f"    Skipping batch {i//BATCH_SIZE + 1} (only {benign_count} benign, {malicious_count} malicious)"
                                )
            ...
            print(
                f"  Completed {file_path.name}. Total samples processed: {total_samples_processed:,}"
            )
            print(
                f"  Cumulative class distribution: {total_benign_seen:,} benign, {total_malicious_seen:,} malicious"
            )
    ...
    print(
        f"Final class distribution seen: {total_benign_seen:,} benign, {total_malicious_seen:,} malicious"
    )
    print(
        f"Class imbalance ratio: {total_benign_seen/total_malicious_seen:.2f}:1 (benign:malicious)"
    )
    ...
    if validation_scores:
        ...
        if final_val:
            print(
                f"Final validation predictions: {final_val['pred_benign']} benign, {final_val['pred_malicious']} malicious"
            )
...
def _prepare_features(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    feature_columns = [
        col for col in COMMON_COLUMNS if col not in {"label_binary", "label_multiclass"}
    ]
    ...
    numeric_cols = [
        col for col in X.columns if col != "protocol"
    ]  # protocol is categorical
    ...
    for col in numeric_cols:
        if X[col].dtype.kind in "biufc":  # numeric types
            ...
            if inf_mask.any():
                print(
                    f"Warning: Column '{col}' still contains {inf_mask.sum()} infinity values"
                )
...
def evaluate_with_existing_model(
    ...
    for threshold in thresholds:
        ...
        print(
            f"  Threshold {threshold}: F1={f1_thresh:.3f}, Acc={acc_thresh:.3f}, Pred dist={pred_dist}"
        )
    ...
    print(
        f"Transfer performance Acc={target_accuracy:.3f} | F1={target_f1:.3f} | Precision={target_precision:.3f} | Recall={target_recall:.3f}"
    )
    if use_incremental:
...
def evaluate_transfer_with_full_training(
    ...
    if cv_f1_mean >= 0:
        print(
            f"Cross-validation F1 (source, 3-fold): {metrics['cv_f1_mean']:.3f} ± {metrics['cv_f1_std']:.3f}"
        )
    else:
...
def main() -> None:
    ...
    with MemoryMonitor("CIC-IDS-2017 Loading"):
        ...
        if use_full:
            cic_sample_path = (
                CIC_DATASET_DIR / "Friday-WorkingHours-Afternoon-DDos.pcap_ISCX.csv"
            )
    ...
    def create_balanced_sample(
        df: pd.DataFrame, max_samples: int = MAX_SAMPLES
    ) -> pd.DataFrame:
    ...
    if nsl_cic_model is not None:
        ...
        results.append(
            evaluate_transfer_with_full_training(
                nsl_sample,
                cic_eval_sample,
                "NSL-KDD",
                "CIC-IDS-2017",
    ...
    with MemoryMonitor("Results Saving"):
        ...
        payload = {
            ...
            "training_method": (
                "incremental_full_dataset" if use_full else "sample_based"
            ),
