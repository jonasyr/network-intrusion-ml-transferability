...
class ScientificVisualizationEnhancer:
                ...
                for dataset_name, df in results_data.items():
                    if metric in df.columns and "model_name" in df.columns:
                        ...
                        for _, row in df.iterrows():
                            ...
                            complexity = model_complexity.get(
                                model_name, 6
                            )  # Default to mid-complexity
                        ...
                        sorted_data = sorted(
                            zip(complexity_scores, performance_scores, model_labels)
                        )
                        ...
                        for j, (comp, perf, label) in enumerate(
                            zip(complexities, performances, labels)
                        ):
                            if (
                                j % 3 == 0 or j == len(labels) - 1
                            ):  # Annotate every 3rd model and the last
                                ax.annotate(
                                    label[:6],
                                    (comp, perf),
                                    fontsize=6,
    ...
    def generate_convergence_analysis(self) -> bool:
        ...
        try:
            ...
            if cv_dir.exists():
                ...
                if cv_files:
                    for cv_file in cv_files:
                        dataset_name = cv_file.stem.replace(
                            "cv_detailed_results_", ""
                        ).replace("cv_detailed_results", "Combined")
                        try:
                            df = pd.read_csv(cv_file)
                            cv_data[dataset_name] = df
                            print(
                                f"✅ Loaded CV results for {dataset_name}: {len(df)} models"
                            )
                        except Exception as e:
            ...
            if not cv_data:
                print(
                    "❌ No detailed cross-validation results found. Run experiment 04 first."
                )
                return False
                    ...
                    for idx, (model_name, row) in enumerate(df.iterrows()):
                        ...
                        try:
                            ...
                            if isinstance(fold_scores_str, str):
                                ...
                                fold_scores = [
                                    float(x.strip())
                                    for x in fold_scores_str.split(",")
                                    if x.strip()
                                ]
                            ...
                            if len(fold_scores) > 1:
                                ...
                                cumulative_mean = np.cumsum(fold_scores) / np.arange(
                                    1, len(fold_scores) + 1
                                )
                                ...
                                ax.plot(
                                    range(1, len(cumulative_mean) + 1),
                                    cumulative_mean,
                                    "o-",
                                    label=model_name_clean,
                                    linewidth=1.5,
                                    markersize=4,
                                    color=color,
                                )
                            ...
                            print(
                                f"⚠️ Error processing fold scores for {model_name}: {e}"
                            )
                            continue
            ...
            # Save the figure
            output_path = (
                self.subdirs["convergence_analysis"] / "cv_convergence_analysis.pdf"
            )
            plt.savefig(output_path, dpi=300, bbox_inches="tight", format="pdf")
            plt.close()
    ...
    def generate_cross_dataset_analysis(self) -> bool:
        ...
        try:
            ...
            cross_results_path = Path(
                "data/results/bidirectional_cross_dataset_analysis.csv"
            )
            ...
            if cross_data:
                ...
                if "bidirectional" in cross_data:
                    ...
                    nsl_to_cic_f1 = (
                        df["CIC_Test_F1"].tolist()
                        if "CIC_Test_F1" in df.columns
                        else [0] * len(models)
                    )
                    cic_to_nsl_f1 = (
                        df["NSL_Test_F1_From_CIC"].tolist()
                        if "NSL_Test_F1_From_CIC" in df.columns
                        else [0] * len(models)
                    )
                    ...
                    models = (
                        nsl_df["Model"].tolist()
                        if "Model" in nsl_df.columns
                        else nsl_df.index.tolist()
                    )
                    nsl_to_cic_f1 = (
                        nsl_df["Target_F1"].tolist()
                        if "Target_F1" in nsl_df.columns
                        else [0] * len(models)
                    )
                    cic_to_nsl_f1 = (
                        cic_df["Target_F1"].tolist()
                        if "Target_F1" in cic_df.columns
                        else [0] * len(models)
                    )
            ...
            if cross_data:
                ...
                if "bidirectional" in cross_data:
                    df = cross_data["bidirectional"]
                    if (
                        "NSL_to_CIC_Relative_Drop" in df.columns
                        and "CIC_to_NSL_Relative_Drop" in df.columns
                    ):
                        models = (
                            df["Model"].tolist()
                            if "Model" in df.columns
            ...
            if "bidirectional" in cross_data:
                ...
                if "Domain_Divergence_forward" in df.columns:
                    ...
                    divergence_reverse = (
                        df["Domain_Divergence_reverse"].tolist()
                        if "Domain_Divergence_reverse" in df.columns
                        else divergence_forward
                    )
                    ...
                    ax3.bar(
                        ...
                        divergence_forward,
                        width,
                        label="NSL→CIC Divergence",
                        color=self.colors["cross_dataset"],
                    )
                    ax3.bar(
                        x + width / 2,
                        divergence_reverse,
                        width,
                        label="CIC→NSL Divergence",
            ...
            if "bidirectional" in cross_data:
                df = cross_data["bidirectional"]
                if (
                    "NSL_to_CIC_Transfer_Ratio" in df.columns
                    and "CIC_to_NSL_Transfer_Ratio" in df.columns
                ):
                    models = (
                        df["Model"].tolist()
                        if "Model" in df.columns
            ...
            # Save the figure
            output_path = (
                self.subdirs["cross_dataset_analysis"]
                / "cross_dataset_transfer_analysis.pdf"
            )
            plt.savefig(output_path, dpi=300, bbox_inches="tight", format="pdf")
            plt.close()
        ...
        try:
            ...
            for i, metric in enumerate(metrics):
                if i < 4 and metric in combined_df.columns:
                    ...
                    grouped_data = (
                        combined_df.groupby(["dataset", "type"])[metric]
                        .mean()
                        .unstack()
                    )
                    grouped_data.plot(
                        ...
                        ax=ax,
                        color=[
                            self.colors["nsl_baseline"],
                            self.colors["nsl_advanced"],
                        ],
                    )
            ...
            if len(metrics) <= 4:
                ...
                if available_metrics:
                    perf_matrix = combined_df.groupby(["dataset", "type"])[
                        available_metrics
                    ].mean()
                    ...
                    if not perf_matrix.empty:
                        ...
                        perf_matrix.plot(
                            ...
                            color=[
                                self.colors["nsl_baseline"],
                                self.colors["nsl_advanced"],
                                self.colors["cic_baseline"],
                                self.colors["cic_advanced"],
            ...
            ax6 = axes[5]
            if (
                "model_name" in combined_df.columns
                and "f1_score" in combined_df.columns
            ):
                model_ranking = (
                    combined_df.groupby("model_name")["f1_score"]
                    .mean()
                    .sort_values(ascending=True)
                )
                if len(model_ranking) > 0:
                    model_ranking.plot(
                        kind="barh", ax=ax6, color=self.colors["primary"]
                    )
                    ...
                    dataset_perf = (
                        combined_df.groupby("dataset")["f1_score"]
                        .mean()
                        .sort_values(ascending=True)
                    )
                    dataset_perf.plot(
            ...
            # Save the figure
            output_path = (
                self.subdirs["comparative_analysis"]
                / "comprehensive_model_comparison.pdf"
            )
            plt.savefig(output_path, dpi=300, bbox_inches="tight", format="pdf")
            plt.close()
    ...
    def generate_timing_analysis(self) -> bool:
        ...
        try:
            ...
            if prediction_times:
                ...
                for bar, value in zip(bars, all_times):
                    ax1.text(
                        ...
                        bar.get_height() + max(all_times) * 0.01,
                        f"{value:.3f}s",
                        ha="center",
                        va="bottom",
                        fontsize=6,
            ...
            if prediction_times:
                ...
                for dataset_name, df in prediction_times.items():
                    ...
                    summary_stats.append(
                        [np.mean(times), np.median(times), np.std(times)]
                    )
            ...
            # Save the figure
            output_path = (
                self.subdirs["comparative_analysis"] / "timing_performance_analysis.pdf"
            )
            plt.savefig(output_path, dpi=300, bbox_inches="tight", format="pdf")
            plt.close()
        ...
        try:
            ...
            if model_consistency:
                ...
                if model_cv:
                    ...
                    bars = ax2.bar(
                        range(len(models)), cv_values, color=self.colors["primary"]
                    )
                    ax2.set_title(
                    ...
                    ax2.set_xticklabels(
                        [m[:10] for m in models], rotation=45, fontsize=8
                    )
                    ax2.grid(True, alpha=0.3)
            ...
            if all_f1_scores:
                ...
                data_for_box = [
                    all_results[name]["f1_score"].values
                    for name in dataset_names
                    if "f1_score" in all_results[name].columns
                ]
            ...
            if model_rankings:
                avg_ranks = {
                    model: np.mean(ranks) for model, ranks in model_rankings.items()
                }
            ...
            for dataset_name, df in all_results.items():
                if all(
                    col in df.columns
                    for col in ["f1_score", "prediction_time", "model_name"]
                ):
            ...
            if summary_data:
                ...
                for i, metric in enumerate(metrics):
                    values = [
                        summary_data[dataset][metric] for dataset in summary_data.keys()
                    ]
                    ax6.bar(
                        x + i * width,
                        values,
            ...
            output_path = (
                self.subdirs["comparative_analysis"]
                / "comprehensive_model_dashboard.pdf"
            )
            plt.savefig(output_path, dpi=300, bbox_inches="tight", format="pdf")
            plt.close()
    ...
    def organize_existing_figures(self) -> bool:
        ...
        try:
            ...
            figure_files = list(paper_figures_dir.glob("*.png")) + list(
                paper_figures_dir.glob("*.pdf")
            )
            ...
            # Copy CSV/TEX tables
            table_files = list(paper_figures_dir.glob("*.csv")) + list(
                paper_figures_dir.glob("*.tex")
            )
    ...
    def generate_scientific_report(self) -> str:
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                "Generated automatically by the Scientific Visualization Enhancer.\n\n"
            )
            ...
            f.write(
                "✅ **Arial/Sans-Serif Fonts**: All visualizations use publication-standard typography\n"
            )
            f.write(
                "✅ **300 DPI PDF Output**: High-quality vector graphics for print\n"
            )
            f.write(
                "✅ **Colorblind-Friendly Palette**: Accessible scientific color scheme\n"
            )
            f.write("✅ **IEEE/ACM Journal Format**: Optimized for 2-column layout\n")
            f.write(
                "✅ **Safe File Operations**: Only reads existing results, no file manipulation\n\n"
            )

            f.write("## Visualization Types\n\n")
            f.write(
                "1. **Learning Curves**: Model performance vs complexity analysis\n"
            )
            f.write(
                "2. **Convergence Analysis**: Cross-validation statistical stability\n"
            )
            ...
            f.write(
                "All generated PDFs are publication-ready and can be directly included in:\n"
            )
...
def main():
    ...
    print("=" * 60)
    print(
        "Generating publication-quality scientific visualizations from existing results..."
    )
    ...
    try:
        ...
        if not enhancer.available_datasets:
            print(
                "❌ No datasets found. Run experiments 02-03 first to generate results."
            )
            return False
