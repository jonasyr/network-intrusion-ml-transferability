...
class MetricsBasedScientificVisualizer:
    ...
    def generate_scientifically_accurate_roc_visualization(self):
        ...
        for dataset_name, df in results.items():
            ...
            try:
                ...
                output_path = (
                    self.subdirs["roc_curves"] / f"{safe_name}_scientific_roc.pdf"
                )
    ...
    def generate_scientifically_accurate_pr_visualization(self):
        ...
        for dataset_name, df in results.items():
            if not all(
                col in df.columns for col in ["precision", "recall", "f1_score"]
            ):
            ...
            try:
                ...
                for idx, (_, row) in enumerate(df_sorted.iterrows()):
                    ...
                    if f1 > 0.9:  # High performance
                        ...
                        precision_points[recall_points > recall] *= (
                            1 - (recall_points[recall_points > recall] - recall) * 2
                        )
                ...
                ax.axhline(
                    ...
                    linewidth=1.5,
                    alpha=0.7,
                    label=f"Baseline (Random = {baseline_precision:.1f})",
                )
                ...
                output_path = (
                    ...
                    / f"{safe_name}_scientific_pr.pdf"
                )
                plt.savefig(output_path, dpi=300, bbox_inches="tight")
                plt.close()
    ...
    def generate_scientifically_accurate_confusion_matrices(self):
        ...
        for dataset_name, df in results.items():
            if not all(
                col in df.columns for col in ["precision", "recall", "accuracy"]
            ):
                continue
            ...
            try:
                ...
                with plt.rc_context({"figure.constrained_layout.use": False}):
                    _, axes = plt.subplots(
                        1, len(top_models), figsize=(5 * len(top_models), 4)
                    )
                    if len(top_models) == 1:
                        axes = [axes]
                ...
                output_path = (
                    ...
                    / f"{safe_name}_scientific_cm.pdf"
                )
                plt.savefig(output_path, dpi=300, bbox_inches="tight")
                plt.close()
    ...
    def generate_cross_validation_statistical_analysis(self):
        ...
        try:
            ...
            for i, metric in enumerate(available_metrics[:4]):
                ...
                cis_lower = (
                    df[f"{metric}_ci_lower"].values
                    if f"{metric}_ci_lower" in df.columns
                    else means - df[f"{metric}_std"].values
                )
                cis_upper = (
                    df[f"{metric}_ci_upper"].values
                ...
                ax.errorbar(
                    ...
                    y_pos,
                    xerr=[
                        means[sorted_indices] - cis_lower[sorted_indices],
                ...
                ax.set_yticks(y_pos)
                ax.set_yticklabels(
                    [models[i].replace("_", " ").title() for i in sorted_indices]
                )
            ...
            output_path = (
                self.subdirs["confusion_matrices"]
                / "cv_statistical_analysis_scientific.pdf"
            )
            plt.savefig(output_path, dpi=300, bbox_inches="tight")
            plt.close()
    ...
    def generate_cross_dataset_transfer_analysis(self):
        ...
        try:
            ...
            for transfer_name, df in transfer_data.items():
                ...
                target_f1 = (
                    df["Target_F1"].tolist() if "Target_F1" in df.columns else []
                )
            ...
            if "NSL→CIC" in transfer_data and "CIC→NSL" in transfer_data:
                ...
                if "Relative_Drop_%" in nsl_df.columns:
                    ...
                    cic_drops = (
                        cic_df["Relative_Drop_%"].tolist()
                        if "Relative_Drop_%" in cic_df.columns
                        else [0] * len(models)
                    )
            ...
            output_path = (
                self.subdirs["confusion_matrices"]
                / "cross_dataset_scientific_transfer.pdf"
            )
            plt.savefig(output_path, dpi=300, bbox_inches="tight")
            plt.close()
