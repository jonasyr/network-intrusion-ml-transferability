...
class FinalVisualizationGenerator:
    ...
    def load_dataset(self, dataset_name: str, sample_size: int = None):
        ...
        try:
            ...
            if dataset_name.lower() == 'nsl-kdd':
                ...
                numeric_features = [f for f in numeric_features if f not in ['label', 'difficulty']]
            ...
            if sample_size is not None and len(df) > sample_size:
                ...
                print(f"📊 Sampled to {sample_size} instances for computational efficiency")
            else:
    ...
    def generate_feature_importance(self, dataset_name: str):
        ...
        try:
            ...
            model, scaler, X_test, y_test = self.train_model_for_visualization(X, y, 'random_forest')
            if model is None:
                return False
            ...
            bars = ax1.barh(range(n_top), top_importances[::-1], color=self.colors['feature_bars'])
            ...
            ax2.plot(range(1, len(cumulative_importance) + 1), cumulative_importance, 
                    'o-', color=self.colors['roc_line'], linewidth=2, markersize=3)
            ...
            output_path = self.subdirs['feature_importance'] / f'{dataset_clean}_random_forest_feature_importance.pdf'
            ...
            csv_path = self.subdirs['feature_importance'] / f'{dataset_clean}_random_forest_feature_importance.csv'
    ...
    def generate_roc_curves(self, dataset_name: str):
        ...
        try:
            ...
            for model_name in ['random_forest']:  # Start with RF, can expand
                model, scaler, X_test, y_test = self.train_model_for_visualization(X, y, model_name)
                if model is None:
                    continue
            ...
            # Plot diagonal line
            ax.plot([0, 1], [0, 1], color=self.colors['diagonal'], 
            ...
            dataset_clean = dataset_name.lower().replace("-", "_")
            output_path = self.subdirs['roc_curves'] / f'{dataset_clean}_models_roc_curves.pdf'
            plt.savefig(output_path, dpi=300, bbox_inches='tight')
            plt.close()
            ...
            for model_name, results in model_results.items():
                for i, (fpr_val, tpr_val) in enumerate(zip(results['fpr'], results['tpr'])):
                    roc_data.append({
                        'dataset': dataset_name,
                        'model': model_name,
                        'fpr': fpr_val,
            ...
            if roc_data:
                ...
                csv_path = self.subdirs['roc_curves'] / f'{dataset_clean}_models_roc_data.csv'
        ...
        try:
            ...
            for model_name in ['random_forest']:  # Start with RF
                ...
                model_results[model_name] = {
                    'precision': precision, 'recall': recall, 'ap': avg_precision
                }
            
            # Plot baseline
            baseline_precision = np.sum(y_test) / len(y_test)
            ax.axhline(y=baseline_precision, color=self.colors['diagonal'], 
            ...
            # Save with specific naming
            dataset_clean = dataset_name.lower().replace("-", "_")
            output_path = self.subdirs['precision_recall_curves'] / f'{dataset_clean}_models_pr_curves.pdf'
            plt.savefig(output_path, dpi=300, bbox_inches='tight')
            plt.close()
            ...
            pr_data = []
            for model_name, results in model_results.items():
                for i, (prec, rec) in enumerate(zip(results['precision'], results['recall'])):
                    pr_data.append({
                        'dataset': dataset_name,
                        'model': model_name,
                        'precision': prec,
            ...
            if pr_data:
                ...
                csv_path = self.subdirs['precision_recall_curves'] / f'{dataset_clean}_models_pr_data.csv'
    ...
    def generate_confusion_matrices(self, dataset_name: str):
        ...
        try:
            ...
            sns.heatmap(cm_normalized, annot=True, fmt='.3f', cmap=self.colors['cm_cmap'], ax=ax2)
            ...
            # Save with specific naming
            dataset_clean = dataset_name.lower().replace("-", "_")
            output_path = self.subdirs['confusion_matrices'] / f'{dataset_clean}_random_forest_confusion_matrix.pdf'
            plt.savefig(output_path, dpi=300, bbox_inches='tight')
            plt.close()
            ...
            csv_path = self.subdirs['confusion_matrices'] / f'{dataset_clean}_random_forest_classification_report.csv'
    ...
    def generate_cross_dataset_confusion_matrices(self):
        ...
        try:
            ...
            print("⚡ Using samples for cross-dataset analysis (computational efficiency)")
            nsl_X, nsl_y, nsl_features, _ = self.load_dataset('NSL-KDD', sample_size=10000)
            cic_X, cic_y, cic_features, _ = self.load_dataset('CIC-IDS-2017', sample_size=10000)
            ...
            common_feature_count = min(len(nsl_features), len(cic_features), 20)  # Use top 20 common features
            ...
            model_nsl = RandomForestClassifier(n_estimators=50, random_state=42, n_jobs=-1)
            ...
            sns.heatmap(cm_nsl, annot=True, fmt='d', cmap=self.colors['cm_cmap'], ax=axes[0,0])
            ...
            sns.heatmap(cm_nsl_cic, annot=True, fmt='d', cmap=self.colors['cm_cmap'], ax=axes[0,1])
            axes[0,1].set_title('NSL → CIC (Transfer)')
            axes[0,1].set_xlabel('Predicted')
            axes[0,1].set_ylabel('True')
            
            # Scenario 3: CIC trained, CIC tested (baseline)
            model_cic = RandomForestClassifier(n_estimators=50, random_state=42, n_jobs=-1)
            ...
            sns.heatmap(cm_cic, annot=True, fmt='d', cmap=self.colors['cm_cmap'], ax=axes[1,1])
            ...
            sns.heatmap(cm_cic_nsl, annot=True, fmt='d', cmap=self.colors['cm_cmap'], ax=axes[1,0])
            ...
            # Save
            output_path = self.subdirs['confusion_matrices'] / 'cross_dataset_confusion_matrices.pdf'
            plt.savefig(output_path, dpi=300, bbox_inches='tight')
            plt.close()
    ...
    def generate_summary_report(self, success_count: int, total_tasks: int):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write("Essential publication components for ML network anomaly detection research.\n\n")
            ...
            f.write("✅ **Feature Importance**: Shows which network features are most predictive\n")
            f.write("✅ **ROC Curves**: Standard binary classification performance visualization\n") 
            f.write("✅ **Precision-Recall Curves**: Critical for imbalanced datasets (attacks are rare)\n")
            f.write("✅ **Confusion Matrices**: Detailed performance breakdown including cross-dataset transfer\n\n")
