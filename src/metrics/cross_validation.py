...
class CrossValidationFramework:
    ...
    def evaluate_model_cv(
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(self.cv.split(X, y), 1):
            ...
            print(
                f"   📁 Processing fold {fold_idx}/{self.n_folds}...",
                end=" ",
                flush=True,
            )
            ...
            print(
                f"✅ {fold_elapsed:.1f}s (Acc: {cv_results['test_accuracy'][-1]:.3f})"
            )
        ...
        results = {"model_name": model_name, "n_folds": self.n_folds}
        ...
        print(
            f"✅ {model_name}: Accuracy {results['accuracy_mean']:.4f} ± {results['accuracy_std']:.4f}"
        )
    ...
    def create_cv_summary_table(self, model_results: List[Dict]) -> pd.DataFrame:
        ...
        if not df.empty and "Accuracy" in df.columns:
            df["_accuracy_mean"] = [
                float(acc.split(" ± ")[0]) for acc in df["Accuracy"]
            ]
            df = df.sort_values("_accuracy_mean", ascending=False).drop(
                "_accuracy_mean", axis=1
            )

        return df

    def plot_cv_results(
        self, model_results: List[Dict], save_path: str = None
    ) -> plt.Figure:
        """
...
"""
    ...
    def save_results(self, output_dir: str):
        ...
        detailed_results.to_csv(
            output_path / f"cv_detailed_results{dataset_suffix}.csv", index=False
        )

        # Save summary table
        summary_df = self.create_cv_summary_table(list(self.results.values()))
        summary_df.to_csv(
            output_path / f"cv_summary_table{dataset_suffix}.csv", index=False
        )
        ...
        if len(self.results) > 1:
            ...
            comparison_df.to_csv(
                output_path / f"statistical_comparison{dataset_suffix}.csv", index=False
            )
    ...
    def load_existing_results(self, output_dir: str) -> List[str]:
        ...
        if detailed_file.exists():
            try:
                ...
                print(
                    f"📋 Found {len(completed_models)} existing results: {completed_models}"
                )
                ...
                for _, row in existing_df.iterrows():
                    ...
                    for col in result_dict.keys():
                        if col.endswith("_scores") and isinstance(
                            result_dict[col], str
                        ):
                            try:
...
def run_cic_cross_validation():
    ...
    try:
        ...
        if cic_data is not None:
            ...
            print(
                f"📊 Class distribution: Normal={np.sum(y == 0)}, Attack={np.sum(y == 1)}"
            )
        else:
            print(
                f"❌ CIC {dataset_type} loading failed, skipping CIC cross-validation"
            )
            return None, []
    except Exception as e:
        ...
        try:
            cic_data = preprocessor.load_data(
    ...
    remaining_models = {
        name: path for name, path in model_paths.items() if name not in completed_models
    }

    if completed_models:
        print(
            f"✅ Resuming: {len(completed_models)} models already done, {len(remaining_models)} remaining"
        )
    else:
...
def run_full_cross_validation():
    ...
    with MemoryMonitor("Data Preprocessing"):
        ...
        print(
            f"📊 Memory mode: {'Full dataset' if config['use_full_dataset'] else 'Optimized'}"
        )
    ...
    try:
        ...
        for result in cv_results:
            print(
                f"   {result['model_name']}: Accuracy={result['accuracy_mean']:.4f}±{result['accuracy_std']:.4f}"
            )
        return cv_framework, cv_results

    # Create visualizations
    print("\n📊 Creating visualizations...")
    fig = cv_framework.plot_cv_results(
        cv_results, "data/results/cv_results_boxplot.png"
    )
