...
class ScientificFigureGenerator:
    ...
    def create_figure1_performance_overview(self) -> plt.Figure:
        ...
        has_datasets = (
            "dataset" in all_results.columns
            and len(all_results["dataset"].unique()) > 1
        )
        ...
        ax1.set_title(
            "(a) Model Performance Ranking", loc="left", fontweight="bold", pad=10
        )
        ...
        if has_datasets:
            for i, dataset in enumerate(datasets):
                ...
                ax2.scatter(
                    dataset_data["recall"],
                    dataset_data["precision"],
                    c=self.qual_colors[i],
                    label=dataset,
                    s=30,
                    alpha=0.7,
                    edgecolors="black",
        ...
        if has_datasets and len(datasets) == 2:
            # Box plot comparison
            data_by_dataset = [
                all_results[all_results["dataset"] == d]["f1_score"] for d in datasets
            ]
            bp = ax3.boxplot(
                data_by_dataset,
                labels=datasets,
        ...
        # Save figure
        plt.savefig(
            self.output_dir / "figure1_performance_overview.pdf",
            dpi=300,
            bbox_inches="tight",
        )
        plt.savefig(
            self.output_dir / "figure1_performance_overview.png",
            dpi=300,
            bbox_inches="tight",
        )
    ...
    def create_figure2_statistical_analysis(self) -> plt.Figure:
        ...
        fig.suptitle(
            "Statistical Analysis with Confidence Intervals", fontweight="bold", y=0.95
        )
        ...
        for i, (dataset, df) in enumerate(cv_results.items()):
            ...
            ax.set_yticks(y_pos)
            model_labels = [
                model.replace("_", " ").title() for model in top_models["Model"]
            ]
            ...
            for j, (mean, lower, upper) in enumerate(
                zip(top_means, top_lower, top_upper)
            ):
                if j == 0:  # Best model
                    ax.text(
                        0.02,
        ...
        # Save figure
        plt.savefig(
            self.output_dir / "figure2_statistical_analysis.pdf",
            dpi=300,
            bbox_inches="tight",
        )
        plt.savefig(
            self.output_dir / "figure2_statistical_analysis.png",
            dpi=300,
            bbox_inches="tight",
        )
