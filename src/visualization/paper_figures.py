...
sns.set_style(
    "whitegrid", {"grid.linestyle": "-", "grid.linewidth": 0.5, "grid.alpha": 0.5}
)
...
class PaperFigureGenerator:
    ...
    def load_dataset_results(self, dataset: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if not baseline_results.empty:
            baseline_results["category"] = baseline_results["model_name"].apply(
                categorize_model
            )
            baseline_results["dataset"] = dataset.upper()

        if not advanced_results.empty:
            advanced_results["category"] = advanced_results["model_name"].apply(
                categorize_model
            )
    ...
    def load_all_results(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        # Combine results
        combined_baseline = (
            pd.concat(all_baseline, ignore_index=True)
            if all_baseline
            else pd.DataFrame()
        )
        combined_advanced = (
            pd.concat(all_advanced, ignore_index=True)
            if all_advanced
            else pd.DataFrame()
        )
    ...
    def create_model_performance_comparison(
        ...
        fig, axes = plt.subplots(
            2, 2, figsize=(16, 12)
        )  # Better for 2-column LaTeX format
        ...
        fig.suptitle(
            f"Machine Learning Models Performance Comparison\n{dataset_title}",
            fontsize=14,
            fontweight="bold",
        )
        ...
        all_results["display_name"] = all_results.apply(
            lambda row: f"{row['model_name'].replace('_', ' ').title()} ({row['dataset']})",
            axis=1,
        )
        ...
        colors = [
            category_colors.get(cat, self.colors["neutral"])
        ...
        axes[0, 0].set_title(
            "Model Accuracy Comparison", fontweight="bold", fontsize=10
        )
        ...
        high_performers = all_results[
            (all_results["precision"] >= 0.8) & (all_results["recall"] >= 0.8)
        ]
        low_performers = all_results[
            (all_results["precision"] < 0.8) | (all_results["recall"] < 0.8)
        ]
        ...
        axes[1, 0].set_title(
            "Precision vs Recall (Top & Bottom Models Labeled)",
            fontweight="bold",
            fontsize=10,
        )
        ...
        if "roc_auc" in all_results.columns:
            ...
            axes[1, 1].set_title(
                "Model Categories Distribution", fontweight="bold", fontsize=10
            )
        ...
        legend_elements = [
            plt.Rectangle(
                (0, 0), 1, 1, facecolor=color, edgecolor="white", label=category
            )
            for category, color in category_colors.items()
            if category in all_results["category"].values
        ]
        fig.legend(
        ...
        if save_path is None:
            if self._has_both_datasets():
                ...
                dataset_prefix = "cic"
            save_path = (
                self.output_dir / f"{dataset_prefix}_model_performance_comparison.png"
            )
    ...
    def create_attack_distribution_analysis(
        self, save_path: Optional[str] = None
    ) -> plt.Figure:
        """
        ...
        axes[1, 0].barh(
            range(len(train_attack_types)),
            train_attack_types.values,
        ...
        for i, v in enumerate(train_attack_types.values):
            axes[1, 0].text(
                v + max(train_attack_types.values) * 0.01,
                i,
                f"{v:,}",
                va="center",
                fontsize=9,
    ...
    def create_cic_attack_distribution_analysis(
        self, save_path: Optional[str] = None
    ) -> plt.Figure:
        """
        ...
        colors_readable = [
            self.colors["success"] if "BENIGN" in label else self.colors["primary"]
            for label in attack_dist.index
        ]
        ...
        axes[0, 0].legend(
            ...
            [
                f"{label}\n({val:,})"
                for label, val in zip(attack_dist.index, attack_dist.values)
            ],
            loc="center left",
        ...
        binary_dist = (
            cic_data["Label"]
            .apply(lambda x: "Normal" if x == "BENIGN" else "Attack")
            .value_counts()
        )
        ...
        sorted_categories = category_dist.sort_values(
            ascending=True
        )  # Ascending for horizontal bars

        # Use different colors for Normal vs Attack categories
        bar_colors = [
            self.colors["success"] if cat == "Normal" else self.colors["secondary"]
    ...
    def create_cross_validation_comparison(
        ...
        fig.suptitle(
            "Cross-Validation Performance Comparison", fontsize=14, fontweight="bold"
        )
        ...
        for i, dataset in enumerate(datasets):
            ...
            axes[i].set_title(
                f"{dataset} Cross-Validation", fontweight="bold", fontsize=10
            )
        ...
        if save_path:
            ...
            plt.savefig(
                self.output_dir / "cross_validation_comparison.png",
                dpi=300,
                bbox_inches="tight",
                facecolor="white",
            )
    ...
    def create_cross_dataset_transfer_analysis(
        self, save_path: Optional[str] = None
    ) -> plt.Figure:
        """
        ...
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(
            "Cross-Dataset Transfer Learning Analysis", fontsize=14, fontweight="bold"
        )
        ...
        if "Bidirectional" in cross_dataset_results:
            ...
            for i, (v1, v2) in enumerate(zip(nsl_cic_ratios, cic_nsl_ratios)):
                axes[0, 0].text(
                    x[i] - width / 2, v1 + 0.02, f"{v1:.2f}", ha="center", fontsize=8
                )
                axes[0, 0].text(
                    x[i] + width / 2, v2 + 0.02, f"{v2:.2f}", ha="center", fontsize=8
                )

            axes[0, 0].set_title(
                "Transfer Ratios by Direction", fontweight="bold", fontsize=10
            )
        ...
        if "NSL→CIC" in cross_dataset_results:
            ...
            axes[0, 1].set_title(
                "NSL→CIC Transfer Performance", fontweight="bold", fontsize=10
            )
        ...
        if "Bidirectional" in cross_dataset_results:
            ...
            axes[1, 0].barh(
                y_pos, sorted_ratios, color=self.colors["accent"], alpha=0.8
            )
            axes[1, 0].set_yticks(y_pos)
            axes[1, 0].set_yticklabels(sorted_models, fontsize=8)
            axes[1, 0].set_title(
                "Average Transfer Performance", fontweight="bold", fontsize=10
            )
            ...
            for i, v in enumerate(sorted_ratios):
                axes[1, 0].text(
                    v + 0.01, i, f"{v:.3f}", va="center", fontsize=8, fontweight="bold"
                )
        ...
        if "Bidirectional" in cross_dataset_results:
            ...
            for i, model in enumerate(bidirectional["Model"]):
                axes[1, 1].annotate(
                    model,
                    (nsl_cic_ratios[i], cic_nsl_ratios[i]),
                    xytext=(3, 3),
                    textcoords="offset points",
                    fontsize=7,
                    alpha=0.8,
            ...
            axes[1, 1].set_title(
                "Transfer Asymmetry Analysis", fontweight="bold", fontsize=10
            )
            ...
            axes[1, 0].text(
                0.5,
                0.5,
                "Cross-dataset analysis data not available",
                ha="center",
                va="center",
            ...
            axes[1, 1].text(
                0.5,
                0.5,
                "Transfer asymmetry data not available",
                ha="center",
                va="center",
        ...
        if save_path:
            ...
            plt.savefig(
                self.output_dir / "cross_dataset_transfer_analysis.png",
                dpi=300,
                bbox_inches="tight",
                facecolor="white",
            )
    ...
    def create_harmonized_evaluation_summary(
        self, save_path: Optional[str] = None
    ) -> plt.Figure:
        """
        ...
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(
            "Experiment 05 vs 06: Standard vs Harmonized Cross-Dataset Evaluation",
            fontsize=14,
            fontweight="bold",
        )
        ...
        if "Bidirectional" in cross_dataset_results and "results" in harmonized_data:
            ...
            axes[0, 0].set_title(
                "NSL-KDD → CIC-IDS-2017 Transfer Performance",
                fontweight="bold",
                fontsize=10,
            )
            axes[0, 0].set_xlabel("Models", fontsize=9)
        ...
        if "Bidirectional" in cross_dataset_results and "results" in harmonized_data:
            ...
            axes[0, 1].set_title(
                "CIC-IDS-2017 → NSL-KDD Transfer Performance",
                fontweight="bold",
                fontsize=10,
            )
        ...
        if "results" in harmonized_data:
            ...
            if cv_performance and target_performance:
                ...
                axes[1, 0].set_title(
                    "Harmonized: CV vs Target Performance",
                    fontweight="bold",
                    fontsize=10,
                )
        ...
        axes[1, 1].set_title(
            "Experimental Comparison Summary", fontweight="bold", fontsize=10
        )
        axes[1, 1].axis("off")
        ...
        if save_path:
            ...
            plt.savefig(
                self.output_dir / "harmonized_evaluation_summary.png",
                dpi=300,
                bbox_inches="tight",
                facecolor="white",
            )
    ...
    def create_dataset_comparison_overview(
        self, save_path: Optional[str] = None
    ) -> plt.Figure:
        """
        ...
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(
            "Dataset Performance Comparison Overview", fontsize=14, fontweight="bold"
        )

        # Get datasets and create better color scheme
        datasets = sorted(all_results["dataset"].unique())
        dataset_colors = {
            datasets[i]: self.qual_colors[i] for i in range(len(datasets))
        }

        # 1. Accuracy comparison by dataset (improved scatter plot)
        for i, dataset in enumerate(datasets):
            dataset_data = all_results[all_results["dataset"] == dataset].reset_index(
                drop=True
            )
            axes[0, 0].scatter(
        ...
        category_dataset = (
            all_results.groupby(["dataset", "category"]).size().unstack(fill_value=0)
        )
        ...
        category_dataset.plot(
            kind="bar", ax=axes[1, 0], color=category_colors_list, width=0.8
        )
        axes[1, 0].set_title(
            "Model Categories by Dataset", fontweight="bold", fontsize=10
        )
        ...
        accuracy_by_dataset = [
            all_results[all_results["dataset"] == d]["accuracy"].values
            for d in datasets
        ]
        ...
        axes[1, 1].set_title(
            "Accuracy Distribution by Dataset", fontweight="bold", fontsize=10
        )
        ...
        if save_path:
            ...
            plt.savefig(
                self.output_dir / "dataset_comparison_overview.png",
                dpi=300,
                bbox_inches="tight",
                facecolor="white",
            )
    ...
    def create_performance_summary_table(
        ...
        if "roc_auc" in all_results.columns:
            summary_table["ROC-AUC"] = all_results["roc_auc"].apply(
                lambda x: f"{x:.4f}"
            )
