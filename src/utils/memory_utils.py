...
def should_use_full_dataset(min_memory_gb: float = 16.0) -> bool:
    ...
    if use_full:
        print(
            f"✅ Using FULL dataset for scientific accuracy (memory: {total_memory:.1f}GB)"
        )
    else:
        print(
            f"⚠️ Using OPTIMIZED dataset due to memory constraints (memory: {total_memory:.1f}GB)"
        )
        print(
            f"   💡 For scientifically accurate results, ensure >16GB RAM and run on full dataset"
        )
...
def get_optimal_sample_size(
    ...
    print(
        f"📊 Optimal sample size: {optimal_samples:,} / {total_samples:,} "
...
class MemoryMonitor:
    ...
    def __enter__(self):
        ...
        print(
            f"🔍 Starting {self.operation_name} (Available: {self.start_memory:.1f}GB)"
        )
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        print(
            f"✅ {self.operation_name} complete (Used: {memory_used:.1f}GB, "
